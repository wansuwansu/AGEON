...
export default function Dashboard() {
  ...
  // 로컬 스토리지에서 데이터 불러오기 (초기 1회)
  useEffect(() => {
    setIsClient(true);
    const savedData = localStorage.getItem("ageon_dashboard_data");
    const savedResources = localStorage.getItem("ageon_resources");

    if (savedData) {
      setPlatforms(JSON.parse(savedData));
    } else {
      setPlatforms([
        {
          id: 1, name: "네이버 스마트스토어", link: "https://sell.smartstore.naver.com/", docLink: "",
          status: "운영 중", email: "master@ageon.co.kr", accountId: "ageon_admin", accountPw: "ageon1234!", manager: "최완수", dueDate: "",
          memo: "[26.03.15] 엑소프로실 브랜드관 세팅 완료\n[26.03.20] 첫 매출 발생",
        }
      ]);
    }
    ...
    if (savedResources) {
      ...
      setResources([
        { id: 1, title: "📄 AGEON 사업자등록증", url: "https://drive.google.com/" },
        { id: 2, title: "📁 엑소프로실 회사소개서(최신)", url: "https://drive.google.com/" },
        { id: 3, title: "💳 법인 통장사본", url: "https://drive.google.com/" },
      ]);
    }
  }, []);
  ...
  useEffect(() => {
    if (isClient) {
      localStorage.setItem("ageon_dashboard_data", JSON.stringify(platforms));
      localStorage.setItem("ageon_resources", JSON.stringify(resources));
    }
  }, [platforms, resources, isClient]);
  ...
  const handleAdd = (e) => {
    ...
    setPlatforms([{ ...formData, id: Date.now() }, ...platforms]);
    setFormData({ name: "", link: "", docLink: "", status: "검토 중", email: "", accountId: "", accountPw: "", manager: "", dueDate: "", memo: "" });
  };

  const handleDelete = (id) => {
    if (confirm("정말 삭제하시겠습니까?")) setPlatforms(platforms.filter(p => p.id !== id));
  };
  ...
  const handleAddResource = (e) => {
    ...
    setResources([{ ...resourceForm, id: Date.now() }, ...resources]);
  ...
  return (
    <div className="min-h-screen bg-[#0B1120] text-slate-300 font-sans selection:bg-cyan-500/30 pb-20 relative">
      ...
      <div className="max-w-6xl mx-auto p-8">
        ...
        <header className="mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
          ...
          <button 
            ...
            className="bg-slate-800/80 hover:bg-slate-700 text-cyan-400 font-bold py-2.5 px-5 rounded-xl border border-cyan-900/50 hover:border-cyan-500/50 transition-all shadow-[0_0_15px_rgba(6,182,212,0.15)] flex items-center gap-2"
          >
            📁 공용 자료실 열기
          </button>
        </header>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-emerald-900/20 backdrop-blur border border-emerald-900/50 p-5 rounded-2xl shadow-lg">
            <h3 className="text-emerald-500 text-xs font-bold tracking-widest mb-1">OPERATING (운영 중)</h3>
          ...
          <div className="bg-purple-900/20 backdrop-blur border border-purple-900/50 p-5 rounded-2xl shadow-lg">
            <h3 className="text-purple-500 text-xs font-bold tracking-widest mb-1">IN PROGRESS (진행 중)</h3>
          ...
          <div className="bg-pink-900/20 backdrop-blur border border-pink-900/50 p-5 rounded-2xl shadow-lg">
            <h3 className="text-pink-500 text-xs font-bold tracking-widest mb-1">ACTION NEEDED (준비/검토)</h3>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-6 bg-slate-900/60 p-4 rounded-2xl border border-slate-800 backdrop-blur-sm">
          <div className="flex w-full md:w-auto gap-4 flex-grow">
            <input 
              type="text" 
              placeholder="🔍 플랫폼명 또는 담당자 검색..." 
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="bg-slate-950 border border-slate-700 text-slate-200 px-4 py-2.5 rounded-xl focus:ring-2 focus:ring-cyan-500 outline-none w-full md:w-80"
            />
            <select 
              value={filterStatus} 
              onChange={(e) => setFilterStatus(e.target.value)}
              className="bg-slate-950 border border-slate-700 text-slate-200 px-4 py-2.5 rounded-xl focus:ring-2 focus:ring-cyan-500 outline-none cursor-pointer"
            >
        ...
        <div className="space-y-4 mb-10">
          {filteredPlatforms.length === 0 ? (
             <div className="text-center p-12 text-slate-500 border border-slate-800 rounded-2xl border-dashed bg-slate-900/30">
                조건에 맞는 데이터가 없습니다.
             </div>
          ) : (
                  ...
                  <div onClick={() => setExpandedId(isExpanded ? null : p.id)} className="p-5 flex items-center justify-between cursor-pointer select-none group relative gap-4">
                    <div className="flex-grow flex flex-col">
                      <div className="flex items-center gap-3 flex-wrap">
                        ...
                        {dDay && (
                          <span className={`px-2 py-0.5 rounded text-[10px] font-bold border whitespace-nowrap ${dDay.color}`}>
                            {dDay.text}
                          </span>
                        )}
                      </div>
                    ...
                    {/* 💥 [수정] 글자 줄바꿈 방지 (shrink-0, whitespace-nowrap 적용) */}
                    <div className="text-right shrink-0">
                      <span className={`text-xs font-bold px-2 py-1.5 rounded-md bg-slate-950 border border-slate-800 whitespace-nowrap ${config.color.replace('bg-', 'text-')}`}>
                        {p.status}
                      </span>
                    </div>

                    <div className="w-6 flex justify-end shrink-0">
                      <div className={`text-lg transition-transform duration-300 ${isExpanded ? 'rotate-180 text-cyan-400' : 'text-slate-500'}`}>
                        ▼
                      </div>
                    </div>
                  </div>

                  {/* 펼쳐진 상세 내용 (이전 코드와 동일, 생략 없이 전체 제공) */}
                  {isExpanded && (
                    <div className="border-t border-slate-800/50 bg-slate-950/50 p-6 grid grid-cols-1 lg:grid-cols-2 gap-8 animate-[fadeIn_0.3s_ease-in-out]">
                      <div className="space-y-5">
                        ...
                        <div className="space-y-2 bg-slate-900 p-4 rounded-xl border border-slate-800">
                          <div className="flex items-center gap-2">
                            ...
                            <input type="url" value={p.link} onChange={(e) => handleUpdateField(p.id, "link", e.target.value)} className="flex-grow bg-slate-950 border border-slate-700 text-xs text-slate-300 px-2 py-1.5 rounded outline-none" placeholder="https://..."/>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <input type="url" value={p.docLink} onChange={(e) => handleUpdateField(p.id, "docLink", e.target.value)} className="flex-grow bg-slate-950 border border-slate-700 text-xs text-slate-300 px-2 py-1.5 rounded outline-none" placeholder="드라이브 링크"/>
                        ...
                        <div className="space-y-2 bg-slate-900 p-4 rounded-xl border border-slate-800">
                          <div className="flex items-center gap-2">
                            ...
                            <input type="email" value={p.email} onChange={(e) => handleUpdateField(p.id, "email", e.target.value)} className="flex-grow bg-slate-950 border border-slate-700 text-xs text-slate-300 px-2 py-1.5 rounded outline-none" placeholder="가입 이메일"/>
                          </div>
                          <div className="flex items-center gap-2">
                      ...
                      <div className="flex flex-col h-full">
                        <div className="flex justify-between items-center mb-2 border-b border-slate-800 pb-2">
                          ...
                          <button onClick={() => handleAddDateLog(p.id, p.memo)} className="text-[10px] font-bold bg-slate-800 hover:bg-slate-700 text-cyan-400 border border-slate-700 px-2 py-1 rounded transition-colors">
                            + 오늘 날짜 추가
                          </button>
                        </div>
                        <textarea
                          value={p.memo}
                          onChange={(e) => handleUpdateField(p.id, "memo", e.target.value)}
                          className="w-full flex-grow min-h-[150px] bg-slate-900 border border-slate-800 rounded-xl p-4 text-sm text-slate-300 focus:ring-1 focus:ring-cyan-500/50 outline-none resize-none font-mono leading-relaxed transition-all"
                          placeholder="진행 상황과 특이사항을 기록하세요."
                        />
                        <div className="flex justify-end mt-4">
                           <button onClick={() => handleDelete(p.id)} className="text-xs font-bold text-red-500/70 hover:text-red-400 bg-slate-900 px-4 py-2 rounded-lg transition-colors border border-red-900/50 hover:border-red-500/50">
                             🗑️ 데이터 삭제
                           </button>
                        </div>
                      </div>
        ...
        <div className="bg-slate-900/40 border border-slate-800 p-6 rounded-2xl shadow-xl">
          ...
          <form onSubmit={handleAdd} className="flex flex-wrap gap-3 items-center">
            ...
            <button type="submit" className="bg-cyan-600 hover:bg-cyan-500 text-white font-bold px-6 py-3 rounded-xl shadow-[0_0_15px_rgba(8,145,178,0.4)] transition-all active:scale-95 text-sm whitespace-nowrap">
              + 추가하기
            </button>
          </form>
        </div>
      ...
      {/* 💥 [신규] 공용 자료실 팝업 (모달창) */}
      {isResourceOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-[fadeIn_0.2s_ease-in-out]">
          <div className="bg-[#0f172a] border border-slate-700 w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden">
            <div className="p-5 border-b border-slate-800 flex justify-between items-center bg-slate-900">
              ...
              <button onClick={() => setIsResourceOpen(false)} className="text-slate-400 hover:text-white text-2xl leading-none">&times;</button>
            </div>
            ...
            <div className="p-6">
              <div className="space-y-3 mb-6 max-h-[40vh] overflow-y-auto pr-2 custom-scrollbar">
                ...
                {resources.map(r => (
                  <div key={r.id} className="flex items-center justify-between bg-slate-900/80 border border-slate-800 p-3 rounded-xl group hover:border-slate-600 transition">
                    <div className="flex-grow truncate pr-4">
                      <p className="text-sm font-bold text-slate-200 truncate">{r.title}</p>
                      <a href={r.url} target="_blank" rel="noreferrer" className="text-xs text-blue-400 hover:underline truncate block mt-0.5">{r.url}</a>
                    </div>
                    <div className="flex gap-2 shrink-0">
                      <a href={r.url} target="_blank" rel="noreferrer" className="bg-blue-900/50 hover:bg-blue-800 text-blue-300 px-3 py-1.5 rounded-lg text-xs font-bold transition">
                        열기
                      </a>
                      <button onClick={() => handleDeleteResource(r.id)} className="bg-red-900/20 hover:bg-red-900/50 text-red-400 px-2 py-1.5 rounded-lg text-xs font-bold transition">
                        삭제
                      </button>
                    </div>
                  </div>
              ...
              <form onSubmit={handleAddResource} className="bg-slate-950 p-4 rounded-xl border border-slate-800">
                <h3 className="text-xs font-bold text-slate-500 mb-3">새 자료 추가하기</h3>
                <div className="flex flex-col gap-2">
                  <input type="text" placeholder="자료명 (예: 사업자등록증)" value={resourceForm.title} onChange={(e) => setResourceForm({...resourceForm, title: e.target.value})} className="bg-slate-900 border border-slate-700 text-slate-200 px-3 py-2 rounded-lg text-sm outline-none focus:ring-1 focus:ring-cyan-500"/>
                  <div className="flex gap-2">
                    <input type="url" placeholder="구글 드라이브 등 URL 링크" value={resourceForm.url} onChange={(e) => setResourceForm({...resourceForm, url: e.target.value})} className="flex-grow bg-slate-900 border border-slate-700 text-slate-200 px-3 py-2 rounded-lg text-sm outline-none focus:ring-1 focus:ring-cyan-500"/>
                    <button type="submit" className="bg-cyan-600 hover:bg-cyan-500 text-white font-bold px-4 py-2 rounded-lg text-sm transition shrink-0">
                      등록
                    </button>
                  </div>
                </div>
